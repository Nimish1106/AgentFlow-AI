...
**All 8 phases are complete** (SRS §48).
...
Phase 8 — observability, end-to-end testing, documentation (SRS §42, §45, §49): `app/observability/tracing.py` wires OpenTelemetry across all three tiers — FastAPI requests (auto-instrumented), LangGraph nodes, and MCP tool calls. **Tracing is off by default** (`OTEL_ENABLED=false`): the unit suite, the local venv and any offline run must work with no collector listening, and an observability layer that fails closed when its collector is down would be worse than none. With tracing on and no `OTEL_EXPORTER_OTLP_ENDPOINT`, spans go to the console exporter so the trace tree is visible in `docker compose logs`; set the endpoint to export to any OTLP/HTTP collector. Three invariants hold in the helpers: they **never crash the caller** (a broken tracer degrades to an untraced block — `test_a_broken_tracer_does_not_break_the_block`), they **never suppress a real exception**, and they **never record customer data** (SRS §43 — attributes are ids, durations, counts and status codes; `test_node_spans_carry_ids_and_metrics_only` asserts a customer name and invoice amount cannot reach a span).

Nodes are instrumented at **graph-assembly time**, not per node: `app/graph/instrumentation.py::traced_node` wraps every `add_node` call through a local `add()` helper in `build_workflow_graph`, so a new node cannot be registered untraced by accident. Span attributes come from the node's own `node_executions` entry (Phase 7's channel), so the timing on a span and the timing in `agent_execution_logs` are the same measurement. **A `GraphInterrupt` is control flow, not a failure** (SRS §38) — `span()` checks `GraphBubbleUp` and closes the span cleanly, because OTel's own `__exit__` sets ERROR status when handed any exception, which would have made every human approval look like a fault in the trace.

`tests/test_e2e.py` (21 tests) drives the whole pipeline as one system: `POST /tickets` → Redis stream → `WorkflowConsumer` → `WorkflowRunner` → the compiled graph → HITL interrupt → `POST /approvals/{id}` → resume job → response → dispatcher → persisted resolution. Real: the FastAPI app, both services, the queue envelope, the consumer including consumer-group and acknowledgement semantics, the runner, every graph node and edge, the checkpointer, and the MCP runtime with its audit writes. Substituted, because a unit suite cannot require them: the Groq LLM (`ScriptedLLM` keyed by output schema), Qdrant (`RecordingRetriever`), and Redis (`FakeRedisStream`, which implements XADD/XREADGROUP/XACK plus consumer groups and a pending set — without pending tracking the ACK policy could not be asserted at all). These prove the *system wiring*; they cannot prove the Groq model behaves well, which no offline test can. Coverage from the E2E file alone: 83–100% of the runner, graph nodes and agents.

Two test-design notes worth keeping. **Audit rows cannot be ordered by `timestamp`**: SQLite's `CURRENT_TIMESTAMP` has one-second resolution and Postgres' `now()` is transaction-scoped, so rows written in quick succession share a value, and `id` is a random UUID4. An earlier `ORDER BY timestamp, id` passed alone and failed in the full suite (the run got faster, all five rows landed in the same second, ordering went random). `load_audit_events` therefore returns rows unordered and `test_the_full_lifecycle_is_auditable` asserts sequence through the workflow's *observable state at each step* — which is a stronger assertion anyway: it proves the reviewer's decision is durable **before** the resume job is consumed. **The parallel-safety allowlist is derived, not hardcoded**: `tests/test_agents.py` reads GraphState's reducer annotations, so an unreduced key added to a parallel agent fails the test instead of surfacing as a runtime `InvalidUpdateError`.

Verified on a cold `docker compose down` → `up --build`: all 7 services boot, migrations reach `0004 (head)`, `/health` is ok, the dashboard serves on :3000, **zero tracebacks** across backend/dispatcher/enterprise-mcp/frontend, and tracing is correctly silent when disabled. `docker compose down` leaves no containers or networks behind. With `OTEL_ENABLED=true` in-container, spans carry the full §42 attribute set under the right `service.name`. A ticket submitted through the nginx proxy ran the full nine-node chain, paused at HITL, and completed on approval. Suite: **400 passing, 92% coverage**, stable across repeated full runs.

**Row fan-out bug found and fixed during Phase 8** (introduced in Phase 7's `list_tickets`). Both the subscription tier and the latest-workflow lookup were plain outer joins, so a customer holding two subscriptions — an upgrade history — returned that customer's ticket **twice**, with `total` still reporting 1. The workflow join had the same defect and was subtler: `_latest_workflow_subquery` matched on `MAX(started_at)`, and two runs created in one transaction share `now()` exactly (Postgres' `now()` is transaction-scoped), so a retried ticket duplicated. Both are now correlated scalar subqueries (`_tier_subquery`, `_latest_workflow_subquery`) with a deterministic tiebreak, so a ticket is always exactly one row. `TestNoRowFanOut` covers both fan-out shapes plus the page-overflow case; verified live against Postgres, where row count, unique ids and `total` all agree.

**Optimization: measured, not assumed.** The live trace of a real duplicate-charge workflow reads: supervisor 4548ms, task_planner 1ms, billing_agent 11920ms (5 tool calls), policy_agent 672ms, aggregator/risk_engine/dispatcher 0ms, response_agent 2285ms. Nearly all wall-clock is Groq inference inside the reasoning agents; the deterministic governance nodes are already free. The one structural inefficiency visible here is the billing agent's five tool calls for one invoice question — that is prompt/tool-loop behaviour (`agent_max_tool_rounds` is 8), not a code path that can be optimised without changing what the agent does. Reducing it means tightening the Billing prompt or the tool descriptions and re-measuring, which is a behavioural change and was left alone rather than guessed at. What Phase 8 *did* add is the instrumentation to see this at all: per-node timings in `agent_execution_logs`, on the dashboard timeline, and on OTel spans.
...
OTEL_ENABLED=true docker compose up                    # tracing on; spans to container logs
OTEL_ENABLED=true OTEL_EXPORTER_OTLP_ENDPOINT=http://jaeger:4318 docker compose up   # export to a collector
pytest tests/test_e2e.py -v                            # the whole pipeline as one system
